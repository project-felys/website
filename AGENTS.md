...
- Feature pages stay view-only. The chat surface is driven by `lib/chat/useChatSession.ts`, which owns the transcript, the explicit `ChatStatus`, and the single async pump that consumes the line stream; `useTypewriter.ts` owns the line in the input box and `lib/useBackendHealth.ts` — the one shared root-level hook — owns the backend probe (used by both chat and voice). Keep view concerns (scrolling, focus) in `app/[locale]/chat/page.tsx` and session concerns in the hook — do not reintroduce pacing state as loose refs in the page.
- **Where code lives.** `components/` holds *presentational* UI shared by two or more surfaces (`navigator`, `backgroundImage`, `icons`) and nothing else. `lib/<feature>/` owns one surface end to end, **including that feature's own components** (`lib/voice/waveformProgress.tsx`) and hooks. `lib/config/` is the cross-cutting i18n + endpoints slice; its `configProvider.tsx` is the one client entry point there (the `@/lib/config` barrel stays free of client code, which is why nothing imports it through the barrel).
- **Feature slices must not import each other.** They currently import zero from one another; the only shared targets are `components/`, `lib/config/`, and `lib/useBackendHealth.ts`. Share through one of those instead of reaching across, and do not merge `components/` into `lib/` — the separate root is exactly what makes a cross-boundary import obvious in review.
...
- **Functions** are verb-first (`sseToLineStream`, `configureMonaco`, `makeTtsFilename`, `postChatCompletion`). Converters read `<input>To<output>`: `floatsToWav`, `pcmBufferToWav`, `perplexityToOpacity`. Hooks are `useXxx` in a matching `useXxx.ts`.
...
- **Booleans** — internal state, refs and flag locals are prefixed `is` / `has` / `can`: `isVisible`, `isDraggingRef`, `isDisposedRef`, `hasStartedRef`. Three deliberate exceptions: component props stay bare adjectives (`disabled`, `blurred`), a ref that mirrors a value keeps the source name (`messagesRef`), and wire-format fields (`stream_audio`, `success`) must never be renamed — they are the server/WASM protocol.
